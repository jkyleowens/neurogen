...
class SensoryEncoder(nn.Module):
    ...
    def __init__(self, input_size, encoding_size, num_encoders=4, sparsity=0.1):
        ...
        # NEW: Add dedicated projection to match expected interface
        self.output_projection = nn.Linear(encoding_size, encoding_size)
        
        # NEW: Type detection capability for direct interface with InputProcessor
        self.type_detector = nn.Sequential(
            nn.Linear(input_size, input_size // 2),
            nn.ReLU(),
            nn.Linear(input_size // 2, 4)  # 4 types: numerical, categorical, text, mixed
        )
    ...
    def forward(self, x):
        ...
        for t in range(seq_length):
            ...
            # NEW: Apply final output projection
            sparse_encoded = self.output_projection(sparse_encoded)
...
class InputProcessor(nn.Module):
    ...
    def detect_input_type(self, x):
        """
        Neuromorphic input type detection with robust handling of tensor dimensions.
        
        This completely reimplemented version avoids matrix multiplications that were 
        causing shape mismatches, replacing the neural network classifier with
        a stable statistical approach.
        """
        with torch.no_grad():  # Critical: prevent gradient tracking here
            batch_size, seq_length, _ = x.size()
            device = x.device
            
            # Calculate basic statistics
            means = x.mean(dim=(0, 1))
            variances = x.var(dim=(0, 1))
            
            # Initialize masks for each type
            numerical_mask = torch.ones(self.input_size, device=device)
            categorical_mask = torch.zeros(self.input_size, device=device)
            text_mask = torch.zeros(self.input_size, device=device)
            mixed_mask = torch.zeros(self.input_size, device=device)
            
            # Calculate unique values for each feature
            for i in range(self.input_size):
                # Safe sampling to avoid memory issues
                sample_size = min(100, batch_size * seq_length)
                sample_indices = torch.randint(0, batch_size * seq_length, (sample_size,))
                sample = x.reshape(-1, self.input_size)[sample_indices, i]
                
                try:
                    unique_count = torch.unique(sample).numel()
                    
                    # Features with very low variance and few unique values are likely categorical
                    if variances[i] < 0.01 and unique_count <= 10:
                        categorical_mask[i] = 0.8
                        numerical_mask[i] = 0.2
                        
                    # Features with high variance and many unique values might be text-like
                    elif variances[i] > 0.1 and unique_count > 20:
                        text_mask[i] = 0.7
                        numerical_mask[i] = 0.3
                        
                    # Everything else is treated as primarily numerical
                    else:
                        numerical_mask[i] = 0.9
                        mixed_mask[i] = 0.1
                        
                except RuntimeError:
                    # If unique calculation fails, default to numerical
                    numerical_mask[i] = 1.0
            
            # Create fixed type probabilities based on feature distribution
            numerical_count = numerical_mask.sum().item()
            categorical_count = categorical_mask.sum().item()
            text_count = text_mask.sum().item()
            mixed_count = mixed_mask.sum().item()
            total = max(1.0, numerical_count + categorical_count + text_count + mixed_count)
            
            type_probs = torch.tensor([
                numerical_count / total,
                categorical_count / total, 
                text_count / total,
                mixed_count / total
            ], device=device)
        ...
        # Update internal state with smoothing
    ...
    def forward(self, x, adapt=True):
        """
        Forward pass of the Input Processor with robust gradient handling.
        ...
        try:
            # Use the robust type detection without neural networks
            with torch.no_grad():
                type_probs, type_masks = self.detect_input_type(x)
                
                # Preprocess different types of data (with gradient safety)
                ...
                # Combine preprocessed data using type probabilities to focus on relevant preprocessing
                combined_x = (
            ...
            print("Using raw input as fallback")
            combined_x = x
            type_probs = torch.tensor([0.25, 0.25, 0.25, 0.25], device=x.device)
            type_masks = {
                'numerical': torch.ones(self.input_size, device=x.device),
                'categorical': torch.zeros(self.input_size, device=x.device),
                'text': torch.zeros(self.input_size, device=x.device),
                'mixed': torch.zeros(self.input_size, device=x.device)
            }
        
        # Extract features (this part requires gradients)
        try:
            features, attention_weights = self.feature_extractor(combined_x)
            
            # Adapt feature extraction if requested (safely)
            if adapt:
                with torch.no_grad():
                    feature_mean = features.mean(dim=(0, 1), keepdim=True)
                    feature_std = features.std(dim=(0, 1), keepdim=True) + 1e-5
            ...
            # Encode features (this part requires gradients)
            encoded, encoder_weights = self.encoder(features)
            
            # Project to output size (this part requires gradients)
            output = self.output_projection(encoded)
            
            # Store in memory buffer for online learning (safely without gradients)
            if adapt:
                with torch.no_grad():
                    self.memory_buffer.append((x.detach().clone(), output.detach().clone()))
            ...
            # Collect metadata
            metadata = {
                'type_probs': type_probs.detach().clone(),
                'attention_weights': attention_weights.detach().clone(),
                'encoder_weights': encoder_weights.detach().clone(),
                'features': features.detach().clone()
            }
            
        except Exception as e:
            print(f"Error in feature extraction/encoding: {e}")
            print("Using simple linear projection fallback")
            # Emergency fallback: apply a simple linear transformation
            output = F.linear(
                x.reshape(batch_size * seq_length, -1),
                torch.ones(self.output_size, x.size(-1), device=x.device),
                torch.zeros(self.output_size, device=x.device)
            ).reshape(batch_size, seq_length, self.output_size)
            
            metadata = {
                'type_probs': type_probs,
                'attention_weights': torch.ones(batch_size, seq_length, device=x.device),
                'encoder_weights': torch.ones(batch_size, seq_length, 4, device=x.device) / 4,
                'features': x
            }
    ...
    def type_detection_fallback(self, x):
        """
        Fallback heuristic for type detection when neural detection fails.
        ...
        # Calculate simple statistics directly without neural network
        means = x.detach().mean(dim=(0, 1))
        variances = x.detach().var(dim=(0, 1))
        
        # Create simple heuristic masks
        numerical_mask = torch.ones(self.input_size, device=device) * 0.8  # Assume mostly numerical data
        categorical_mask = torch.zeros(self.input_size, device=device)
        text_mask = torch.zeros(self.input_size, device=device)
        mixed_mask = torch.ones(self.input_size, device=device) * 0.2
        
        # Detect likely categorical features (low variance)
        for i in range(self.input_size):
            if variances[i] < 0.01:
                categorical_mask[i] = 0.7
                numerical_mask[i] = 0.2
        
        # Create fixed type probabilities
        type_probs = torch.tensor([0.7, 0.2, 0.0, 0.1], device=device)  # [numerical, categorical, text, mixed]
